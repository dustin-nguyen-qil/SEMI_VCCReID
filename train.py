...
def train(config, epoch, model, classifier, clothes_classifier, criterion_cla,
          criterion_pair, criterion_clothes, criterion_adv, optimizer,
          optimizer_cc, trainloader, pid2clothes):
    ...
    for batch_idx, (imgs, pids, camids, clothes_ids) in enumerate(trainloader):
        ...
        imgs, pids, clothes_ids, pos_mask = imgs.cuda(), pids.cuda(
        ), clothes_ids.cuda(), pos_mask.float().cuda()
        ...
        clothes_corrects.update(
            torch.sum(clothes_preds == clothes_ids.data).float() /
            clothes_ids.size(0), clothes_ids.size(0))
    ...
    logger.info('Epoch{0} '
                ...
                'Acc:{acc.avg:.2%} '
                'CloAcc:{clo_acc.avg:.2%} '.format(epoch + 1,
