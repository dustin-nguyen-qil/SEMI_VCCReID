...
# if CONFIG.TEST.TEST_SET == CONFIG.TEST.TRAIN_SET:
#     model_name = build_model_name()
#     state_dict_path = osp.join(CONFIG.METADATA.SAVE_PATH, model_name) # replace path here if using pretrained SEMI 
# else:
state_dict_path = "work_space/save/vccr_120_0.0005_16_c2dres50.pth" # replace path here if using pretrained SEMI 
model_name = state_dict_path.split('/')[-1]

print(f"Testing Model: {model_name} on {CONFIG.TEST.TEST_SET}")
...
if CONFIG.DATA.DATASET in ['vccr', 'ccvid']:
    ...
    path = f"work_space/output/{model_name[:-4]}_{CONFIG.TEST.TEST_SET}.png"
    plt.legend()
    plt.savefig(path)
else:
